...
Game::Game(std::size_t Screen_Width, std::size_t Screen_Height, std::size_t grid_width, std::size_t grid_height)
    : snake(grid_width, grid_height),
      engine(dev()),
       //populating the unique pointer with an instance of the Renderer
...
Game::~Game()
{
  //joining back the treads to before closing the program
...
void Game::Run(std::size_t target_frame_duration) {
  ...
  while (running) {
    ...
    Update();
    //changing the render to adapt with the new unique pointer for the render
...
/*
  Adding function to change the direction of the snake after recieving the signal from the controlled thread 
*/
