...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        auto pToken = std::find_if(tokens.begin(), tokens.end(), [](const std::pair<std::string, std::string> &pair) { return pair.first == "PARENT"; }); // parent token
                        auto cToken = std::find_if(tokens.begin(), tokens.end(), [](const std::pair<std::string, std::string> &pair) { return pair.first == "CHILD"; });  // child token

                        if (pToken != tokens.end() && cToken != tokens.end())
                        {
                            // get iterator on incoming and outgoing node via ID search
                            auto parentNode = std::find_if(_nodes.begin(), _nodes.end(), [&pToken](std::unique_ptr<GraphNode> &node) { return node->GetID() == std::stoi(pToken->second); });
                            auto childNode = std::find_if(_nodes.begin(), _nodes.end(), [&cToken](std::unique_ptr<GraphNode> &node) { return node->GetID() == std::stoi(cToken->second); });
    ...
    for (auto i = std::begin(_nodes); i != std::end(_nodes); ++i)
    {
        // search for nodes which have no incoming edges
        if ((*i)->GetNumberOfParents() == 0)
        {

            if (rootNode == nullptr)
            {
                rootNode = i->get(); // assign current node to root
            }
            else
            {
